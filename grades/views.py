...
from .models import Course, Enrollment, UserProfile, Teacher, CourseComment
from .forms import CourseForm, EnrollForm, RegistrationForm, UserProfileForm, TeacherCourseForm, EnrollmentScoreForm, CourseCommentForm
...
def course_comments(request, course_id):
    """查看課程的所有留言"""
    course = get_object_or_404(Course, id=course_id)
    comments = course.comments.select_related('student').all()
...
def edit_comment(request, comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, '留言已成功更新！')
            return redirect('grades:course_comments', course_id=comment.course.id)
    else:
...
def delete_comment(request, comment_id):
    ...
    comment.delete()
    messages.success(request, '留言已成功刪除！')
    return redirect('grades:course_comments', course_id=course_id)
